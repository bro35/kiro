        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                    this.sendButton(m.chat, `[❗] *Limit Ayang mulia abis, Beberapa Command Tidak Bisa Di Akses*`, author, null, [['Buy Limit', '/buy limit'], ['Menu', '/menu']] , m)
 ...
             if (chat.welcome) {
                ...
                for (let user of participants) {
                    ...
                    try {
                        ...
                        text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Metdatang ayang, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || 'unknow') :
                            (chat.sBye || this.bye || conn.bye || 'Dadah Asu!, @user!')).replace('@user', await this.getName(user))
                        //this.sendFile(id, pp, 'pp.jpg', text, null, false, { mentions: [user] })
    this.sendHydrated(id, text, wm + '\n\n' + botdate, pp, sgc, (action == 'add' ? '💌 WELCOME' : '🐾 BYE'), user.split`@`[0], '🌹 USER', [
      ['ɪɴᴛʀᴏ', '/intro'],
      [(action == 'add' ? '\n\nWADUH BEBAN GROUP NAMBAH 1 :(' : '\n\nBYE BEBAN! SEMOGA LU TENANG MASBRO :)'), '...'],
      [null, null]
...
function ucapan() {
  ...
  if (time >= 10) {
    res = "Selamat Siang woy ☀️"
  }
  if (time >= 15) {
    res = "Selamat Sore asu 🌇"
  }
  if (time >= 18) {
    res = "Malam ayang 🌙"
  }
